...
"""
Regressor based on a cost function given by a Pearson correlation coefficient
"""

__all__ = ['LBFGSPearsonRegressor']
...
def theano_corrcoef(X):
    """Returns correlation coefficients in Theano"""
    Xm = (X.T - X.mean(1).T).T
    Xmn = (Xm.T / tensor.sqrt((Xm ** 2.).sum(1)).T).T
    cc = tensor.dot(Xmn, Xmn.T)
    return cc

def normalize_pearson_like(X, eps=1e-6):
    assert X.ndim == 2
    axis = 1
    rows, cols = X.shape
    ...
    np.putmask(X_div, X_div < eps, eps)
    out = X_num / X_div
    return out
